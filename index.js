...
const DATA_STORAGE_KEY = 'studyPlannerData';
...
const setState = (newState) => {
  ...
  try {
    ...
    localStorage.setItem(DATA_STORAGE_KEY, JSON.stringify(plannerData));
  } catch(e) {
...
const handleResetData = () => {
    if (window.confirm('정말로 모든 공부 기록을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
  ...
  document.querySelectorAll('[data-action]').forEach(el => {
    ...
    switch(action) {
        ...
        case 'upload':
            ...
            break;
        case 'reset':
            el.addEventListener('click', handleResetData);
            break;
    }
  });
    ...
    try {
        ...
        if (savedData) {
            const parsedData = JSON.parse(savedData);
             setState({ ...state, ...parsedData, currentPage: 'main' });
        } else {
             setState({ ...state, currentPage: 'main' });
        }
    } catch(e) {
